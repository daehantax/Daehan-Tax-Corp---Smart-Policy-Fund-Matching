...
// ── 자격 조건 문단만 보는 플래그 ─────────────────────────────────────────────
// 위 FLAG_RULES 는 본문 전체를 보지만, 아래 플래그들은 잘못 붙으면 사장님에게 "자격이
// 없다"는 잘못된 신호를 주므로 자격 문단만 본다.
// 기업마당 사업개요는 "…공고합니다. ☞ <지원대상> ☞ <지원내용> …" 형태라
// 제목 + 지원대상 + 첫 ☞ 구간이 곧 자격 조건이다.
//   측정 근거: '여성' 언급 31건 중 대표자 조건은 13건이었는데, 본문 전체를 보면
//   "여성기업 우대" 한 줄 때문에 운전자금·육성기금 공고까지 딸려 왔다.
function eligibilityText(title, target, summaryFull) {
  const s = String(summaryFull || '');
  const first = s.indexOf('☞');
  const second = first >= 0 ? s.indexOf('☞', first + 1) : -1;
  const targetSection = first < 0 ? '' : s.slice(first, second > 0 ? second : Math.min(s.length, first + 300));
  return [title, target, targetSection].filter(Boolean).join(' ');
}

/**
 * 자격 문단이 "A, B, C 등" / "A 또는 B" 처럼 여러 유형을 나열하는 경우, 우리 키워드가
 * 그중 하나에 걸려도 "그 유형 전용"이 아니다. 실제로 이렇게 오탐이 났다.
 *   "지식서비스산업, 문화콘텐츠산업, 벤처기업, 청년창업기업 등"  → 콘텐츠·청년창업 전용 아님
 *   "소상공인 또는 여성기업"                                → 여성기업이 아니어도 신청 가능
 * 반면 "자동차산업 영위기업", "☞ 뿌리기업", "여성기업 육성사업"은 나열이 아니라 조건이다.
 */
function isEnumerated(text, keyword) {
  const i = text.indexOf(keyword);
  if (i < 0) return false;
  const before = text.slice(Math.max(0, i - 12), i);
  const after = text.slice(i + keyword.length, i + keyword.length + 12);
  return /[,ㆍ·]\s*$/.test(before) || /또는\s*$/.test(before)
    || /^[가-힣]{0,4}\s*(등|,|ㆍ|·|또는)/.test(after);
}

// 자격 문단에서만 찾고 나열형은 배제하는 플래그들 — 잘못 붙으면 "자격 없다"는
// 잘못된 신호를 주기 때문이다. 고객사 쪽 대조 규칙은 앱의
// services/matchingService.ts INDUSTRY_MISMATCH_RULES / OWNER_MISMATCH_RULES 에 있다
// (플래그 이름을 맞출 것 — services/syncParity.test.ts 가 대조한다).
//
// ⚠ '만 39세 이하'는 대표자 조건 키워드에 넣지 않는다. "만 39세 이하 청년 여성을 채용할
//   기업", "만 39세 이하의 미취업자를 신규 채용"처럼 채용 조건에도 쓰여 오탐이 난다.
//   '청년'을 한 덩어리로 두지 않는 이유도 같다 — 74건 중 36건이 채용 지원이다.
const ELIGIBILITY_FLAG_RULES = [
  // 사업자 형태. "개인사업자 또는 법인사업자"처럼 둘 다 허용하는 문구는 나열 배제 규칙이
  // 자동으로 걸러낸다(양쪽 모두 '또는'에 붙어 있으므로).
  ['대상:법인',    ['법인사업자', '법인만', '법인 기업', '「상법」상 법인', '법인등기']],
  ['대상:개인',    ['개인사업자', '개인 사업자', '개인기업']],
  ['제조업',      ['제조업', '제조기업', '제조 기업', '소공인']],
  ['공장보유',    ['공장등록증', '공장 등록증', '공장을 보유', '공장 보유']],
  ['청년창업',    ['청년창업', '청년 창업', '청년 기업가']],
  ['여성기업',    ['여성기업', '여성 기업', '여성창업', '여성 창업', '여성CEO', '여성 대표']],
  // 특정 산업 분야 전용. 방산(공고 26건)은 넣지 않았다 — 업태·종목으로 방산 기업을
  // 식별할 수 없어(고객사 0개사) 규칙에 넣으면 전 고객사에게 오탐이 난다.
  ['산업:자동차', ['자동차산업', '자동차부품', '자동차 부품', '미래모빌리티']],
  ['산업:조선',   ['조선산업', '조선해양', '중소조선']],
  ['산업:섬유',   ['섬유산업', '섬유패션']],
  ['산업:식품',   ['식품산업', '농식품산업', '식품제조']],
  ['산업:반도체', ['반도체산업', '반도체 산업']],
  ['산업:바이오', ['바이오산업', '바이오헬스', '의료기기산업']],
  ['산업:화학',   ['석유화학']],
  ['산업:뿌리',   ['뿌리산업', '뿌리기업']],
  ['산업:콘텐츠', ['콘텐츠산업', '게임산업']],
  ['산업:관광',   ['관광산업', '관광ㆍ서비스업', '관광기업']],
  ['산업:예술',   ['예술산업', '예술 분야', '예술분야', '공연산업']],
];

// 명시 테이블에 없는 산업 제한을 잡아내기 위한 일반 패턴.
// 산업 제한은 롱테일이다 — 방위산업 5건, 이차전지분야 3건, 수소산업 2건, 조명산업 2건…
// 개별 규칙을 계속 늘리는 대신, "업종 제한이 있는데 우리가 판정하지 못했다"고 표시한다.
// (고객사 업태·종목 어휘와 공고 어휘가 달라 동의어 표를 만들어야만 판정이 되고,
//  그 표 없이 단어만 대조하면 실제 해당 기업에게도 주의가 붙는다. 예: '뿌리산업'은
//  업종에 '금속·주조·도금'으로 나타나므로 '뿌리'만 찾으면 오탐이다.)
// 두 경로로 찾는다. 자격 문단 전체를 느슨하게 훑으면 171건이 걸리는데 절반이 오탐이었다
// ("기술사업화 유공자 포상"의 '포상분야', "환경형 예비사회적기업"의 '환경분야' 등).
//   ① 제목의 '○○산업|○○업종' — 제목에 산업명이 있으면 그 자체로 강한 신호
//      ('분야'는 제외 — "모집분야·포상분야"처럼 산업과 무관한 쓰임이 많다)
//   ② 자격 문단의 '○○산업|업종|분야 + 대상 명사' — "예술 분야 사업자", "자동차산업 영위기업"
const INDUSTRY_HINT_TITLE_RE = /([가-힣A-Za-z]{2,8})(산업|업종)/g;
const INDUSTRY_HINT_TARGET_RE = /([가-힣A-Za-z]{2,8})(산업|업종|분야|업계)\s*(?:관련\s*)?(?:기업|사업자|업체|영위|소재|해당하는|속하는)/g;
const INDUSTRY_HINT_NOISE = /^(해당|관련|주력|지역|전략|주요|기타|특정|동일|모든|각|전|신|본|타|미래|첨단|혁신|중점|핵심|유망|지원|참여|사업|기업|모집|한국표준산업|한국산업|국가산업|산업|창업|특화|연고|시군구연고산업|지식|서비스|제조|이상|목표|우대|신청|포상|대상|밀집|전체|다양)/;

// 우리가 판정할 수 없는 자격 조건 — 카드에 "공고문 확인 필요"로 정직하게 표시한다.
// 자격 문단 전수 조사 결과 조건은 14종이었고, 그중 DB 데이터가 없어 판정 불가한 것들이다.
//   상시근로자 수 121건 / 업력 73건 / 매출액 62건 / 수출실적 29건 / 업종코드 24건 / 인증 16건
// 나열형 배제를 적용하지 않는다 — "매출액, 근로자수 기준 충족"처럼 나열돼 있어도
// 조건이 실제로 존재하기 때문이다.
const UNVERIFIABLE_FLAG_RULES = [
  ['조건:근로자수', [/상시\s*근로자/, /근로자\s*\d+인/, /\d+인\s*(이상|미만|이하)/]],
  ['조건:매출액',   [/매출액/, /연매출/, /평균매출/]],
  ['조건:인증',     [/인증\s*기업/, /벤처기업\s*확인/, /이노비즈/, /메인비즈/, /인증서/]],
  ['조건:4대보험',  [/4대\s*(사회)?보험/, /고용보험\s*가입/]],
  ['조건:체납',     [/체납/, /납세증명/]],
];
...
function extractTargetFlags(title, target, summaryFull) {
  ...
  // 업종·대표자·산업 조건은 자격 문단만 보고, 나열형("A, B 등")은 배제한다
  const eligible = eligibilityText(title, target, summaryFull);
  for (const [flag, keywords] of ELIGIBILITY_FLAG_RULES) {
    if (keywords.some(k => eligible.includes(k) && !isEnumerated(eligible, k))) flags.add(flag);
  }

  // 판정할 수 없는 조건은 "공고문 확인 필요"로 표시하기 위해 그대로 담는다
  for (const [flag, patterns] of UNVERIFIABLE_FLAG_RULES) {
    if (patterns.some(re => re.test(eligible))) flags.add(flag);
  }

  // 명시 테이블로 판정한 산업이 없는데 산업 제한 표현이 있으면 "확인 필요"로 알린다
  if (![...flags].some(f => f.startsWith('산업:'))) {
    const found = [
      ...String(title || '').matchAll(INDUSTRY_HINT_TITLE_RE),
      ...eligible.matchAll(INDUSTRY_HINT_TARGET_RE),
    ].some(m => !INDUSTRY_HINT_NOISE.test(m[1]) && !isEnumerated(eligible, m[0]));
    if (found) flags.add('조건:업종제한');
  }
